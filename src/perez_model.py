...
from util import col_perez_a, col_perez_b, col_perez_sky, col_aoi_deg, col_solar_zen_ang, col_perez_sky, col_dni, col_ghi, col_dhi, col_perez_air_mass
...
def perez(df: pd.DataFrame, tiltangle: float):
    ...
    df[col_perez_air_mass] = df.apply(lambda x: get_perez_air_mass(x[col_solar_zen_ang], x[col_perez_b]))

    if zenith <= 93.5:
        perez_AirMass_optical = (math.cos(math.radians(perez_b)) + (
                    0.15 * (93.15 - zenith)) ** -1.253) ** -1
    else:
